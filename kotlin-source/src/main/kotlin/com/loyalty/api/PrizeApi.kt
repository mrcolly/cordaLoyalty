...
class PrizeApi(private val rpcOps: CordaRPCOps) {
    ...
    fun getAllPrizesByParams(@DefaultValue("1") @QueryParam("page") page: Int,
        ...
        try{
            ...
            val results = builder {
                ...
                val results = rpcOps.vaultQueryBy<PrizeState>(
                        criteria,
    ...
    @Produces(MediaType.APPLICATION_JSON)
    fun createProposal(req : PrizePojo): Response {

        try {
            val eni : Party = rpcOps.wellKnownPartyFromX500Name(CordaX500Name.parse("O=Eni,L=Milan,C=IT"))!!

            val prize = rpcOps.startTrackedFlow(PrizeFlow::Creator,
                    eni,
                    req)
                    .returnValue.getOrThrow()

            val resp = ResponsePojo("SUCCESS","transaction committed to ledger.", prize)
